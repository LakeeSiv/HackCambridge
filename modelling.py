...
'''
Adhi's Working Code Below
'''

def plot_concentration(alcohol_ingested, half_life, height, mass, age, gender, interval=60): # ensure all units in seconds
    '''
...
def main_calculate(user_info, drinks_list):
    ...
    t = np.arange(0, 24 * 3600, 60)
    c = np.arange(0, 24 * 3600, 60)
    ...
    for drink in drinks_list:
        alcohol_name,time_string, volume = drink

        time_dt = datetime.strptime(start_time, "%H:%M")
        time_diff = start_time_dt - time_dt
        ...
        alcohol_ingested_amount = alcohol_ingested(alcohol_name, volume)
